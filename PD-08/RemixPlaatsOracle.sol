...
//Laat de luchtdruk zien.
contract luchtdrukNederland is usingProvable {
   string  public luchtd;
   ...
   function __callback(bytes32 /* myid prevent warning*/ , string memory result ) override public {
    ...
    //Haalt de luchtdruk op.
   function GetLuchtd() public payable {
       priceOfUrl = provable_getPrice("URL");
       require (address(this).balance >= priceOfUrl,
            "please add some ETH to cover for the query fee");
       provable_query("URL", 
            "json(http://weerlive.nl/api/json-data-10min.php?key=demo&locatie=Amsterdam).liveweer[0].luchtd");
   }
}
