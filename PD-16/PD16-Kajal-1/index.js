...
function setGetallen(){
    var setgegevens = document.getElementById("setgegevens").value;
    console.log(setgegevens);
    contract.methods.setGetallen(setgegevens).send({from: accounts[0]}).then(x => console.log(x));
}

function setGegevens(){
    var setgetal = document.getElementById("setgetal").value;
    console.log(setgetal);
    contract.methods.setGegevens(setgetal).send({from: accounts[0]}).then(x => console.log(x));
}
...
function getGegevens() {
  ...
  function getGetallen() {
    contract.methods.getGetallen().call().then( function( getGetallen ) {
      console.log("getGetallen: ", getGetallen);
      document.getElementById('getgetallen').innerHTML = getGetallen;
    });
  }
